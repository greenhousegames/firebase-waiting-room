...
module.exports = class BattleRoomServer {
  ...
  accept() {
    const promise = new rsvp.Promise((resolve, reject) => {
      ...
      this.waitForAllClientsReady().then(() => startedRef.set(true)).catch(reject);
    });
    return promise;
  }
  ...
  waitInRoom() {
    ...
    const promise = new rsvp.Promise((resolve, reject) => {
      this.waitForAllClients().then(uids => {
        this._uids = uids;
        this.master.notify('battle accepted');

        // signal game to start
        this.roomRef.child('ready').set(true).then(() => {
  ...
  sendInvite() {
    const query = this.master.ref.child('waiting').orderByChild('invite').startAt(false).endAt(false);
    this._refs.push(query);
    const promise = new rsvp.Promise((resolve, reject) => {
      ...
      query.on('child_added', snapshot => {
        ...
        if (val.size === this.master.config.size) {
          // send invite
          this.master.ref.child('waiting').child(uid).child('invite').set(roomId).then(() => resolve(uid)).catch(reject);
        }
      }, this);
    });
